...
class plot_2y(Base):
    ...
    def plot(self):
        ...
        assert len(self.data) == 2  # only support 2 data
        # plot figure
        cl = self._kwargs.get("color_list", None)
        ...
        self.ax.plot(d["x"], d["y"], color=cl[i], linewidth=lw, ls=ls)
        ...
        lw = lws[i] if lws is not None else 2
        ls = lss[i] if lss is not None else "-"
        self.ax2.plot(d["x"], d["y"], color=cl[i], linewidth=lw, ls=ls)
        ...
        if self.y2label is not None:
            self.ax2.set_ylabel(self.y2label, hConfig.label_font)
